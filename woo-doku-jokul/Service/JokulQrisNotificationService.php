...
class DokuQrisNotificationService
{
    ...
    public function getQrisNotification($request)
    {
        ...
        $dokuUtils->doku_log('Qris Notify','Component Words Qris Current : ' . $words_to_validate);

        // Melakukan hash dengan SHA1
        $validateWord = sha1($words_to_validate);
        $dokuUtils->doku_log('Qris Notify','Validated Words Qris Current : ' . $validateWord);
        $dokuUtils->doku_log('Qris Notify','Words Qris Expected : ' . $words);
